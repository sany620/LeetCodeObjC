...
- (void)addColor:(UIColor *)color substring:(NSString *)substring withFont:(float) fontvalue
{
    ...
    NSDictionary *attributes1 = @{NSFontAttributeName:[UIFont systemFontOfSize:fontvalue],NSParagraphStyleAttributeName:paragraphStyle};
    NSRange range = [self.string rangeOfStringNoCase:substring];
    [self addAttributes:attributes1 range:range];
    [self addColor:color substring:substring];
}
...
+ (NSMutableAttributedString *)addFontForSubtextsWithText:(NSString *)text fontAndSubtexts:(NSDictionary *)fontAndSubtexts{
    ...
    NSMutableAttributedString *tempText = [[NSMutableAttributedString alloc] initWithString:text];
    [fontAndSubtexts enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        [tempText addFont:obj substring:key];
    }];
    return tempText;
}
